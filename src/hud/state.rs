const EXT_MAP: &[(&[&str], fn() -> ItemKind)] = &[
    ...
    (
        &[
            "rs", "py", "js", "ts", "c", "cpp", "h", "java", "go", "rb", "swift", "sh", "html",
            "css", "json", "yaml", "yml", "toml", "xml", "sql",
        ],
        || ItemKind::Code,
    ),
    (
...
impl ItemKind {
    ...
    pub fn serialize(&self) -> String {
        match self {
            Self::Text => "text".into(),
            Self::Color(h) => format!("color:{h}"),
            Self::File(n) => format!("file:{n}"),
    ...
    pub fn is_inline(&self) -> bool {
        matches!(self, Self::Text | Self::Color(_) | Self::Image)
    }
...
impl ClipboardItem {
    ...
    pub fn display_name(&self) -> String {
        if self.kind.is_inline() {
            return format!("{}  {}", self.app_name, self.relative_time());
        }
        let name = self
            .content
            .rsplit('/')
