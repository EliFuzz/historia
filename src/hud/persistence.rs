...
pub fn init() {
    STORE.with(|s| *s.borrow_mut() = Some(DiskStore::open(data_path())));
}
pub fn active_indices() -> Vec<usize> {
    STORE.with(|s| {
        s.borrow()
            .as_ref()
            .map_or(Vec::new(), |st| st.active.clone())
    })
}
pub fn read_item(idx: usize) -> Option<ClipboardItem> {
    STORE.with(|s| s.borrow().as_ref().and_then(|st| st.read_at(idx)))
}
pub fn add_item(
    content: String,
    app_name: String,
    kind: ItemKind,
    image_data: Option<(&str, &[u8])>,
) {
    with_store(|st| st.add(content, app_name, kind, image_data));
}
pub fn remove_by_id(id: usize) {
    with_store(|st| st.remove(id as u32));
}
pub fn clear() {
    with_store(|st| st.clear());
}
pub fn enforce_limits(max_items: usize, max_age_secs: Option<u64>) {
    with_store(|st| st.enforce_limits(max_items, max_age_secs));
}
pub fn search(query: &str) -> Vec<usize> {
    STORE.with(|s| {
        s.borrow()
            .as_ref()
            .map_or(Vec::new(), |st| st.search(query))
    })
}
...
pub fn load_blob_data(path: &str) -> Option<(String, Vec<u8>)> {
    ...
    Some((
        ItemKind::blob_uti(path.rsplit('.').next().unwrap_or("bin")).into(),
        data,
    ))
}

pub fn blobs_dir() -> PathBuf {
    super::settings::exe_dir().join("blobs")
}
fn data_path() -> PathBuf {
    super::settings::exe_dir().join("clipboard.jsonl")
}
fn with_store(f: impl FnOnce(&mut DiskStore)) {
    STORE.with(|s| {
...
impl DiskStore {
    ...
    fn read_at(&self, idx: usize) -> Option<ClipboardItem> {
        ...
        Some(ClipboardItem {
            ...
            kind: ItemKind::deserialize(&e.kind),
            blob_path: e.blob,
        })
    }
    ...
    fn remove(&mut self, id: u32) {
        if let Some(m) = self.index.iter().find(|m| m.id == id) {
            if let Some(e) = self
                .line(m)
                .and_then(|l| serde_json::from_slice::<Entry>(l).ok())
            {
                if let Some(ref p) = e.blob {
    ...
    fn enforce_limits(&mut self, max_items: usize, max_age: Option<u64>) {
        ...
        if let Some(age) = max_age {
            ...
            for m in &self.index {
                if !self.deleted.contains(&m.id) && m.captured_at < cutoff {
        ...
        if self.active.len() > max_items {
            self.active[max_items..].iter().for_each(|&i| {
                self.deleted.insert(self.index[i].id);
            });
            changed = true;
        }
        if changed {
    ...
    fn search(&self, query: &str) -> Vec<usize> {
        ...
        self.active
            ...
            .filter_map(|&i| {
                ...
                if !ci_contains(line, qb) {
                    return None;
                }
                serde_json::from_slice::<Entry>(line)
                    .ok()
                    .filter(|e| e.content.to_lowercase().contains(&ql))
                    .map(|_| i)
            })
            .collect()
    }
    ...
    fn refresh_mmap(&mut self) {
        self.mmap = File::open(&self.path)
            .ok()
            .filter(|f| f.metadata().is_ok_and(|m| m.len() > 0))
            .and_then(|f| unsafe { Mmap::map(&f).ok() });
    }

    fn line(&self, m: &Meta) -> Option<&[u8]> {
        self.mmap.as_ref().and_then(|mm| self.line_in(mm, m))
    }
    ...
    fn maybe_compact(&mut self) {
        ...
        if d > 0 && (d >= COMPACTION_THRESHOLD || d > self.index.len() / 2) {
            self.compact();
        }
    }
    ...
    fn compact(&mut self) {
        ...
        for m in &self.index {
            let Some(line) = self.line_in(mmap, m) else {
                continue;
            };
            if self.deleted.contains(&m.id) {
                if let Ok(e) = serde_json::from_slice::<Entry>(line) {
                    blobs.extend(e.blob);
                }
                continue;
            }
...
fn scan(path: &PathBuf) -> (Vec<Meta>, Option<Mmap>, u32) {
    let Some(mmap) = File::open(path)
        .ok()
        .filter(|f| f.metadata().map_or(false, |m| m.len() > 0))
        .and_then(|f| unsafe { Mmap::map(&f).ok() })
    else {
    ...
    for line in mmap.split(|&b| b == b'\n') {
        ...
        entries.push(Meta {
            id: nid,
            offset: (line.as_ptr() as usize - base) as u64,
            len: line.len() as u32,
            captured_at: extract_ts(line),
        });
        nid += 1;
    }
