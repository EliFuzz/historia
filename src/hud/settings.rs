...
pub struct Settings {
    ...
    pub items_limit: ItemsLimit,
}

impl Default for Settings {
    fn default() -> Self {
        Self {
            retention_period: RetentionPeriod::ThirtyDays,
            items_limit: ItemsLimit::Fifty,
        }
    }
}
...
pub fn init() {
    let s = std::fs::read_to_string(exe_dir().join("settings.json"))
        .ok()
...
pub fn set_retention_period(p: RetentionPeriod) {
    SETTINGS.with(|s| s.borrow_mut().retention_period = p);
    save();
}
pub fn set_items_limit(l: ItemsLimit) {
    SETTINGS.with(|s| s.borrow_mut().items_limit = l);
    save();
}

fn save() {
    SETTINGS.with(|s| {
        if let Ok(j) = serde_json::to_string_pretty(&*s.borrow()) {
            let _ = std::fs::write(exe_dir().join("settings.json"), j);
        }
    });
}
