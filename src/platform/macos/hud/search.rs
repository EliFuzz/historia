...
use crate::platform::macos::objc_utils::{
    self, gray, rgba, symbol_button, view_with_frame, wire_action,
};
...
pub fn create_top_bar(mtm: MainThreadMarker, width: f64, y: f64) -> Retained<NSView> {
    let bar = view_with_frame(
        mtm,
        NSRect::new(
            NSPoint::new(0.0, y),
            NSSize::new(width, SEARCH_H + BAR_PAD * 2.0),
        ),
    );
...
pub fn top_bar_height() -> f64 {
    SEARCH_H + BAR_PAD * 2.0
}

fn search_field(mtm: MainThreadMarker, w: f64) -> (Retained<NSView>, Retained<NSTextField>) {
    let fw = w - BAR_PAD * 3.0 - BTN_SIZE * 2.0 - BAR_PAD;
    let c = view_with_frame(
        mtm,
        NSRect::new(NSPoint::new(BAR_PAD, BAR_PAD), NSSize::new(fw, SEARCH_H)),
    );
    unsafe {
    ...
    tf.setFrame(NSRect::new(
        NSPoint::new(10.0, (SEARCH_H - fh) / 2.0),
        NSSize::new(fw - 20.0, fh),
    ));
    tf.setBezeled(false);
    ...
    set_placeholder(&tf);
    if let Some(d) = super::controller::search_delegate_ref() {
        unsafe {
            let _: () = msg_send![&tf, setDelegate: &*d];
        }
    }
    c.addSubview(&tf);
    (c, tf)
...
fn set_placeholder(tf: &NSTextField) {
    unsafe {
        ...
        let attr_alloc: *mut AnyObject = msg_send![
            objc2::runtime::AnyClass::get(c"NSAttributedString").unwrap(),
            alloc
        ];
