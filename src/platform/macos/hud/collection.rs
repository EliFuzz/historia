...
use objc2_foundation::{
    MainThreadMarker, NSIndexPath, NSObject, NSObjectProtocol, NSPoint, NSRect, NSSize,
};
...
define_class!(
    ...
    unsafe impl NSCollectionViewDataSource for DataSource {
        ...
        fn count(&self, _: &NSCollectionView, _: NSInteger) -> NSInteger {
...
impl DataSource {
    fn new(mtm: MainThreadMarker, card_size: NSSize) -> Retained<Self> {
        let this = Self::alloc(mtm).set_ivars(DSIvars {
            card_size: RefCell::new(card_size),
    ...
    pub fn remove_item_by_id(&self, id: usize) {
    ...
    pub fn set_filter(&self, q: &str) {
        *self.ivars().filter.borrow_mut() = q.to_owned();
        self.refresh();
    }
    ...
    pub fn enforce_limits(&self, max: usize, age: Option<u64>) {
        crate::hud::persistence::enforce_limits(max, age);
        self.refresh();
    }

    fn refresh(&self) {
        let q = self.ivars().filter.borrow().to_lowercase();
        *self.ivars().filtered.borrow_mut() = if q.is_empty() {
            crate::hud::persistence::active_indices()
        } else {
            crate::hud::persistence::search(&q)
        };
    }
}

pub fn card_width(w: f64) -> f64 {
    (w - PAD * 2.0 - GAP * (VISIBLE_CARD_COUNT as f64 - 1.0)) / VISIBLE_CARD_COUNT as f64
}
pub fn card_height(w: f64) -> f64 {
    card_width(w).mul_add(0.75, 0.0).clamp(80.0, 160.0)
}
...
pub fn create_scroll_view(
    ...
    let cv = NSCollectionView::initWithFrame(
        NSCollectionView::alloc(mtm),
        NSRect::new(NSPoint::ZERO, frame.size),
    );
    cv.setCollectionViewLayout(Some(&layout));
    let id = objc2_app_kit::NSUserInterfaceItemIdentifier::from_str("CardItem");
    unsafe {
        cv.registerClass_forItemWithIdentifier(Some(CardItem::class()), &id);
        let _: () = msg_send![&cv, setWantsLayer: true];
    }
    cv.setBackgroundColors(Some(&objc2_foundation::NSArray::from_retained_slice(&[
        NSColor::clearColor(),
    ])));
    ...
    let sv = NSScrollView::initWithFrame(
        ...
        NSRect::new(
            NSPoint::new(PAD, PAD),
            NSSize::new(frame.size.width - PAD * 2.0, ch),
        ),
    );
