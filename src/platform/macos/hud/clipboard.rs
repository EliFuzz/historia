...
impl ClipboardMonitor {
    fn new(mtm: MainThreadMarker) -> Retained<Self> {
        ...
        let this = Self::alloc(mtm).set_ivars(MonitorIvars {
            change_count: Cell::new(cc),
        });
...
unsafe fn pasteboard() -> *mut AnyObject {
...
fn capture(pb: *mut AnyObject) {
    unsafe {
        ...
        let has = |u: &str| -> bool {
            let ns = NSString::from_str(u);
            msg_send![types, containsObject: &*ns]
        };
        let app = app_name();
        if has(UTI_FILE_URL) {
            return capture_file(pb, &app);
        }
        if has(UTI_TIFF) || has(UTI_PNG) {
            return capture_image(pb, has(UTI_PNG), &app);
        }
        if has(UTI_PLAIN_TEXT) {
            capture_text(pb, &app);
        }
    }
}
...
unsafe fn capture_file(pb: *mut AnyObject, app: &str) {
    unsafe {
        ...
        let dec: *mut AnyObject =
            msg_send![&*(url as *const NSString), stringByRemovingPercentEncoding];
        let s: &NSString = if dec.is_null() {
            &*(url as *const NSString)
        } else {
            &*(dec as *const NSString)
        };
        let raw = s.to_string();
...
unsafe fn capture_image(pb: *mut AnyObject, has_png: bool, app: &str) {
    unsafe {
        ...
        super::controller::add_clipboard_item(
            format!("Image ({})", format_bytes(len)),
            app.to_owned(),
            ItemKind::Image,
            Some((uti.into(), bytes)),
        );
