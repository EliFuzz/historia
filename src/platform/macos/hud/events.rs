...
unsafe extern "C" {
    ...
    fn InstallEventHandler(
        t: *mut c_void,
        h: unsafe extern "C" fn(*mut c_void, *mut c_void, *mut c_void) -> i32,
        n: u64,
        l: *const EventTypeSpec,
        ud: *mut c_void,
        o: *mut *mut c_void,
    ) -> i32;
...
pub fn setup_event_monitors() -> Vec<Retained<ProtocolObject<dyn NSObjectProtocol>>> {
    register_hotkey();
    [mouse_monitor(), key_monitor()]
        .into_iter()
        .flatten()
        .collect()
}

unsafe extern "C" fn hotkey_cb(_: *mut c_void, _: *mut c_void, _: *mut c_void) -> i32 {
...
fn register_hotkey() {
    unsafe {
        ...
        RegisterEventHotKey(
            ...
            0x0300,
            EventHotKeyID {
                signature: u32::from_be_bytes(*b"HST!"),
                id: 1,
            },
            target,
            0,
...
fn mouse_monitor() -> Option<Retained<ProtocolObject<dyn NSObjectProtocol>>> {
    let block = RcBlock::new(|_: NonNull<NSEvent>| {
        if controller::is_panel_visible() {
            controller::hide_panel();
        }
    });
    let raw: *mut AnyObject = unsafe {
        msg_send![AnyClass::get(c"NSEvent")?, addGlobalMonitorForEventsMatchingMask: MASK_LMOUSE, handler: &*block]
    };
    if raw.is_null() {
        return None;
    }
...
fn key_monitor() -> Option<Retained<ProtocolObject<dyn NSObjectProtocol>>> {
    let block = RcBlock::new(|event: NonNull<NSEvent>| -> Option<NonNull<NSEvent>> {
        if handle_key(unsafe { event.as_ref() }) {
            None
        } else {
            Some(event)
        }
    ...
    let raw: *mut AnyObject = unsafe {
        msg_send![AnyClass::get(c"NSEvent")?, addLocalMonitorForEventsMatchingMask: MASK_KEY_DOWN, handler: &*block]
    };
    if raw.is_null() {
        return None;
    }
...
fn handle_key(e: &NSEvent) -> bool {
    ...
    let Some(idx) = KEYCODES.iter().position(|&k| k == code) else {
        return false;
    };
