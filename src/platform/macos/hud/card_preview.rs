...
pub fn create_image_view(mtm: MainThreadMarker) -> Retained<NSView> {
    let fallback =
        || objc_utils::view_with_frame(mtm, NSRect::new(NSPoint::ZERO, NSSize::new(100.0, 100.0)));
    unsafe {
        let Some(cls) = AnyClass::get(c"NSImageView") else {
            return fallback();
        };
        let alloc: *mut AnyObject = msg_send![cls, alloc];
        let inst: *mut AnyObject =
            msg_send![alloc, initWithFrame: NSRect::new(NSPoint::ZERO, NSSize::new(100.0, 100.0))];
        if inst.is_null() {
            return fallback();
        }
        ...
        Retained::retain(inst)
            .map(|r| Retained::cast_unchecked(r))
...
pub fn configure_preview(view: &NSView, item: &ClipboardItem) {
    match &item.kind {
        ...
        ItemKind::Image => {
            if let Some(t) = get_thumb(item) {
                unsafe {
                    let _: () = msg_send![view, setImage: t];
                }
            }
        }
        ItemKind::Text => {}
...
fn get_thumb(item: &ClipboardItem) -> Option<*mut AnyObject> {
    ...
    THUMBS.with(|c| {
        ...
        q.push_back((item.id, thumb));
        while q.len() > MAX_THUMBS {
            q.pop_front();
        }
    });
    Some(thumb)
...
fn scale(src: *mut AnyObject) -> *mut AnyObject {
    unsafe {
        ...
        let s = (THUMB_MAX / ss.width.max(1.0))
            .min(THUMB_MAX / ss.height.max(1.0))
            .min(1.0);
